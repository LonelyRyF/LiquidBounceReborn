...
@ModuleInfo("Velocity", "sunclient", ModuleCategory.COMBAT)
class Velocity : Module() {
    private val modes = ListValue("Velocity", arrayOf("JumpRester", "Cancel", "Hypixel", "GrimY"), "JumpRester")
    ...
    @EventTarget
    fun onP(e: PacketEvent) {
        var spey = e.packet.unwrap()
        val packet = e.packet.unwrap()

        if (e.packet.unwrap() is SPacketEntityVelocity) {
            spey = e.packet.unwrap() as SPacketEntityVelocity
            ...
            when (modes.get()) {
                ...
                "GrimY" -> {
                    ...
                    if (mc.player!!.getDistance(targets) <= grimCheckRangeValue.get()
                    ) {
                        ...
                        attacked = true
                        (LiquidBounce.moduleManager.getModule(KillAura::class.java) as KillAura).blockingStatus = false
